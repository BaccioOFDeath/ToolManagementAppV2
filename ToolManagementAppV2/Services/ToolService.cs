...
public class ToolService
{
    ...
    public void ImportToolsFromCsv(string filePath, IDictionary<string, string> map)
    {
        ...
        foreach (var line in lines.Skip(1))
        {
            ...
            AddTool(new Tool
            {
                ToolNumber = get("ToolNumber"),
                NameDescription = get("NameDescription"),
    ...
    public void ExportToolsToCsv(string filePath)
    {
        ...
        writer.WriteLine("ToolNumber,NameDescription,Location,Brand,PartNumber,Supplier,PurchasedDate,Notes,Keywords,AvailableQuantity,RentedQuantity,IsCheckedOut");
        SqliteHelper.ExecuteReader(_connString, AllToolsSql, null, MapTool).ForEach(t =>
        {
            var vals = new[]
            {
                t.ToolNumber, t.NameDescription, t.Location, t.Brand, t.PartNumber,
                t.Supplier, t.PurchasedDate?.ToString("s") ?? "",
                t.Notes, t.Keywords, t.QuantityOnHand.ToString(), t.RentedQuantity.ToString(),
                t.IsCheckedOut ? "1" : "0"
            };
