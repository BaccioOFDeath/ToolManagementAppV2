...
namespace InventoryManagementApp.Tests
{
    public class MainWindowResponsiveContractTests
    {
        ...
        public void MainWindow_BoundsSearchAndUserSwitcherForScaledWidths()
        {
            ...
            Assert.DoesNotContain("SearchOnLostKeyboardFocus=\"True\"", xaml, StringComparison.Ordinal);
        ...
        [Fact]
        public void ShellSearchDebouncesTypingAndDoesNotRunOnClickAway()
        {
            ...
            Assert.Contains("ScheduleGlobalSearch(value);", mainViewModel, StringComparison.Ordinal);
            Assert.Contains("_globalSearchDebounceTimer.Tick += OnGlobalSearchDebounceTimerTick;", mainViewModel, StringComparison.Ordinal);
            Assert.Contains("_globalSearchDebounceTimer.Start();", mainViewModel, StringComparison.Ordinal);
            Assert.Contains("async void OnGlobalSearchDebounceTimerTick(object? sender, EventArgs e)", mainViewModel, StringComparison.Ordinal);
            ...
            Assert.DoesNotContain("LostKeyboardFocus=\"SearchTextBox_LostKeyboardFocus\"", searchBarXaml, StringComparison.Ordinal);
            Assert.Contains("public void FocusInput(bool selectAll = true)", searchBarCode, StringComparison.Ordinal);
            Assert.Contains("SearchTextBox.CaretIndex = SearchTextBox.Text.Length;", searchBarCode, StringComparison.Ordinal);
            Assert.Contains("if (e.Key == Key.Tab)", searchBarCode, StringComparison.Ordinal);
            Assert.DoesNotContain("SearchOnLostKeyboardFocusProperty", searchBarCode, StringComparison.Ordinal);
            Assert.DoesNotContain("LostKeyboardFocus", searchBarCode, StringComparison.Ordinal);
        }

        [Fact]
        public void ShellSearchStaysAvailableAcrossPagesForItemLookup()
        {
            var mainViewModel = ReadRepoFile("InventoryManagementApp", "ViewModels", "MainViewModel.cs");
            var xaml = ReadRepoFile("InventoryManagementApp", "MainWindow.xaml");

            Assert.Contains("public bool CanShowShellSearch => CanUseSearchTools", mainViewModel, StringComparison.Ordinal);
            Assert.DoesNotContain("!string.Equals(CurrentPageHeaderKey, \"ManageItems\", StringComparison.Ordinal)", mainViewModel, StringComparison.Ordinal);
        ...
        [Fact]
        public void MainWindow_ContentHostSupportsPageContentAndStaysOutOfPageKeyboardFlow()
        {
            var xaml = ReadRepoFile("InventoryManagementApp", "MainWindow.xaml");
            ...
            Assert.DoesNotContain("<ContentControl Grid.Row=\"2\"", xaml, StringComparison.Ordinal);
