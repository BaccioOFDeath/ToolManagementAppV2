    ...
    public class CustomerManagementViewModelTests
    {
        ...
        public void SelectedCustomer_PopulatesNewCustomerFields()
        {
            ...
            try
            {
                var db = new DatabaseService(dbPath);
                ICustomerService customerService = new CustomerService(db);
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer
                {
                    Company = "ACME",
                    Email = "a@b.com",
                    Contact = "John",
        ...
        public async Task UpdateCustomerCommand_UpdatesSelectedCustomer()
        {
            ...
            try
            {
                var db = new DatabaseService(dbPath);
                ICustomerService customerService = new CustomerService(db);
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer { Company = "Old" });
                var existing = customerService.GetAllCustomers().First();
                var vm = new CustomerManagementViewModel(customerService, new StubDialogService());
                vm.SelectedCustomer = existing;
        ...
        public async Task SearchCustomersCommand_FiltersCustomers()
        {
            ...
            try
            {
                var db = new DatabaseService(dbPath);
                ICustomerService customerService = new CustomerService(db);
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer { Company = "Alpha" });
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer { Company = "Beta" });
                var vm = new CustomerManagementViewModel(customerService, new StubDialogService());
        ...
        public async Task DeleteCustomerCommand_RemovesSelectedCustomer()
        {
            ...
            try
            {
                var db = new DatabaseService(dbPath);
                ICustomerService customerService = new CustomerService(db);
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer { Company = "ACME" });
                customerService.AddCustomer(new InventoryManagementApp.Models.Domain.Customer { Company = "Beta" });
                var vm = new CustomerManagementViewModel(customerService, new StubDialogService());
                await vm.SearchCustomersCommand.ExecuteAsync(null);
        ...
        public void DeleteCustomerCommand_CanExecuteDependsOnSelection()
        {
            ...
            try
            {
                ...
                vm.SelectedCustomer = new InventoryManagementApp.Models.Domain.Customer { Company = "ACME" };
