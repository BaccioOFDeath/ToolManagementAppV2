...
namespace InventoryManagementApp.Tests
{
    public class SmoothMouseWheelScrollContractTests
    {
        ...
        public void SmoothWheelHandlerUsesReducedDeltasAndNearestScrollableParent()
        {
            ...
            Assert.Contains("const double PixelsPerWheelStep = 42.0;", source, StringComparison.Ordinal);
            Assert.Contains("const double LogicalUnitsPerWheelStep = 0.65;", source, StringComparison.Ordinal);
            Assert.Contains("FindScrollableViewer(source, e.Delta)", source, StringComparison.Ordinal);
            Assert.Contains("scrollViewer.ScrollToVerticalOffset(targetOffset);", source, StringComparison.Ordinal);
